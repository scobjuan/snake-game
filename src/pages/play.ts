...
const randomFoodPositions = () => {
	...
	positionsFood.x = Math.floor(Math.random() * (448 - 2.5))
	positionsFood.y = Math.floor(Math.random() * (400 - 2.5))
}
...
const checkBorderCollition = () => {
	if (positions[0].x > (scene.width - 2.5) || positions[0].x < 0) {
    hasNotLost = false
	}

	if (positions[0].y > (scene.height - 2.5) || positions[0].y < 0) {
    hasNotLost = false
	}
}
